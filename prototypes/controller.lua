...
function lib.entity_frame(filename, x, y)
   ...
   return {
      layers = {
         ...
         {filename = filename, width = 64, height = 64, shift = {0, -0.125}, scale = 0.5, x = x, y = y},
      },
   }
end
...
function lib.add_recipe(result, ingredients)
   data:extend{
      {type = "recipe", name = result, enabled = false, ingredients = ingredients, result = result, result_count = 1},
   }
