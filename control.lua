...
local function on_gui_opened(e)
   ...
   if addons.classes[entity.name].on_gui_create ~= nil then
      addons.classes[entity.name].on_gui_create(eid, entity, game.players[pid])
   end
...
local function on_gui_closed(e)
   ...
   local pid = e.player_index
   local entity = e.entity
   if entity == nil or global.rgid[pid] == nil then
      return
   end
   ...
   if addons.classes[entity.name].on_gui_destroy ~= nil then
      addons.classes[entity.name].on_gui_destroy(eid, entity, game.players[pid])
   end
   global.rgid[pid] = nil
   global.rpid[eid][pid] = nil
   if next(global.rpid[eid]) == nil then
      global.rpid[eid] = nil
   end
end
...
remote.add_interface(
   "pc-docs", {
      informatron_menu = function(data)
         return addons.docs.menu
      end,
      informatron_page_content = function(data)
         local page = addons.docs.pages[data.page_name] or addons.docs.pages["404"]
         return page(data)
      end,
   }
)
local function on_after_load()
   ...
   for i, g in pairs(global.mem) do
      ...
      for j, c in pairs(g) do
         local s = c.signal
         if (s.type == "virtual" and not game.virtual_signal_prototypes[s.name]) or
            (s.type == "item" and not game.item_prototypes[s.name]) then
            print("  " .. i .. ":" .. j .. " cleared memory " .. (s.name or ""))
