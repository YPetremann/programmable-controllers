...
return {
   ["pc-con"] = {
      on_gui_create = function(index, entity, player)
         ...
         local btn = action.add {type = "button", name = "input-" .. index, caption = "Input", style = "back_button"}
         btn.style.minimal_width = 126
         btn = action.add {type = "button", name = "output-" .. index, caption = "Output", style = "forward_button"}
         btn.style.minimal_width = 126
      end,
      on_gui_destroy = function(index, entity, player)
         editor.destroy(player.gui.left, "pc-mem-window")
         editor.destroy(player.gui.left, entity.name .. "-window")
      end,
