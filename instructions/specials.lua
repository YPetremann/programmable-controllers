   ...
   ["pci-Z00"] = {
      ...
      func = function(G, A) -- F0 UNIT V : do unit test V
         if A == 0 then
            ...
            memory.poke(G, cpu.register, {signal = {type = "virtual", name = "pci-00"}, count = 0})
            -- set UNIT test code
            memory.poke(G, cpu.control + 3, {signal = {type = "virtual", name = "pci-Z00"}, count = 0})
            -- instructions
            memory.poke(G, cpu.control + 4, {signal = {type = "virtual", name = "pci-01"}, count = 2})
            memory.poke(G, cpu.control + 5, {signal = {type = "virtual", name = "pci-14"}, count = 2})
            memory.poke(G, cpu.control + 6, {signal = {type = "virtual", name = "pci-10"}, count = 12})
            memory.poke(G, cpu.control + 7, {signal = {type = "virtual", name = "pci-01"}, count = 1})
            memory.poke(G, cpu.control + 8, {signal = {type = "virtual", name = "pci-14"}, count = 1})
            memory.poke(G, cpu.control + 9, {signal = {type = "virtual", name = "pci-00"}, count = 0})
            memory.poke(G, cpu.control + 10, {signal = {type = "virtual", name = "pci-00"}, count = 0})
            -- display if error
            memory.poke(G, cpu.control + 11, {signal = {type = "virtual", name = "pci-Z01"}, count = 15})
            -- ending
            memory.poke(G, cpu.control + 12, {signal = {type = "virtual", name = "pci-Z04"}, count = 15})
            memory.poke(G, cpu.control + 13, {signal = {type = "virtual", name = "pci-0E"}, count = 1})
            memory.poke(G, cpu.control + 14, {signal = {type = "virtual", name = "pci-10"}, count = 3})
            memory.poke(G, cpu.control + 15, {signal = {type = "virtual", name = "signal-green"}, count = 0})
         end
      end,
   },
   ["pci-Z01"] = {
      ...
      func = function(G, A) -- F1 FAIL V : indicate fail on test V
         memory.poke(G, cpu.control + 15, {signal = {type = "virtual", name = "signal-red"}, count = A})
      end,
   },
   ["pci-Z02"] = {
      name = "",
      ...
      func = function(G, A) -- F2 WARN V : indicate warning on test V
         memory.poke(G, cpu.control + 15, {signal = {type = "virtual", name = "signal-yellow"}, count = A})
      end,
   },
   ["pci-Z03"] = {
      name = "",
      ...
      func = function(G, A) -- F3 DONE V : indicate done test V
         memory.poke(G, cpu.control + 15, {signal = {type = "virtual", name = "signal-green"}, count = A})
      end,
   },
   ["pci-Z04"] = {
      name = "",
      ...
      func = function(G, A) -- FF DBG &A : random in &A in %R range
         ...
         if M.signal.type == "virtual" and M.signal.name then
            ret = "[img = virtual-signal/" .. (M.signal.name) .. "]" .. (M.count)
   ...
   ["pci-Z05"] = {
      ...
      func = function(G, A) -- FF DBG &A : random in &A in %R range
         ...
         for B = A, A + 15, 1 do
            ...
            if M.signal.type == "virtual" and M.signal.name then
               ret = ret .. "[img = virtual-signal/" .. (M.signal.name) .. "]" .. (M.count)
            elseif M.signal.type == "item" and M.signal.name then
               ret = ret .. "[img = item/" .. (M.signal.name) .. "]" .. (M.count)
            else
