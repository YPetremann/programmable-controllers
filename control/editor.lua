...
function lib.create(eid, entity, player)
      ...
      if (name ~= "pc-pow") and (name ~= "pc-ext") then
         ...
         for j = 0, config.blocksize - 1, 1 do
            ...
            if addons.pcn[sname] ~= nil then
               code = code .. "\n" .. DEC_N0(k) .. " " .. addons.pcn[sname] .. " " .. tostring(count)
            else
               local t = string.upper(string.sub(type, 1, 1))
               code = code .. "\n" .. DEC_N0(k) .. " " .. t .. ":" .. sname .. " " .. tostring(count)
            end
         end
         if lent == eid then
            le = #code
   ...
   local gui = player.gui.left.add {type = "frame", name = "pc-mem-window", direction = "vertical", caption = "CODE"}
   ...
   local btn = action.add {type = "button", name = "pc-load", caption = "Load", style = "back_button"}
   btn.style.minimal_width = 126
   btn = action.add {type = "button", name = "pc-save", caption = "Save", style = "forward_button"}
   btn.style.minimal_width = 126
end

function lib.destroy(origin, path)
   if origin[path] ~= nil then
      origin[path].destroy()
   end
end
