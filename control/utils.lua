...
function lib.mem_save(e)
   ...
   for line in string.gmatch(e.text, "[^\n]+") do
      local instruction = {string.match(line, "^ *([0-9][0-9][0-9][0-9]) +([A-Z][A-Z][A-Z][#@&%%]) +(-?[0-9]+) *$")}
      local item = {string.match(line, "^ *([0-9][0-9][0-9][0-9]) +I:([^ ]*) +(-?[0-9]+) *$")}
      local virtual = {string.match(line, "^ *([0-9][0-9][0-9][0-9]) +V:([^ ]*) +(-?[0-9]+) *$")}
      if addons.rpcn[instruction[2]] then
         if instruction[2] == "NOP#" and instruction[3] == "0" then
            memory.poke(eid, tonumber(instruction[1]), {signal = {type = "item"}, count = 0})
         else
            memory.poke(
               eid, tonumber(instruction[1]),
               {signal = {type = "virtual", name = addons.rpcn[instruction[2]]}, count = tonumber(instruction[3])}
            )
         end
      elseif game.item_prototypes[item[2]] then
         memory.poke(eid, tonumber(item[1]), {signal = {type = "item", name = item[2]}, count = tonumber(item[3])})
      elseif game.virtual_signal_prototypes[virtual[2]] then
         memory.poke(
            eid, tonumber(virtual[1]), {signal = {type = "virtual", name = virtual[2]}, count = tonumber(virtual[3])}
         )
   ...
   for _, fid in pairs(global.grp[gid]) do
      ...
      if addons.classes[entity.name].on_save_mem then
         ...
         addons.classes[entity.name].on_save_mem(fid, entity, global.mem[fid])
         lib.read_status(fid, false)
...
function lib.remap(entity, mode) -- OK
   ...
   if mode then
      ...
      if next(global.grp[gid]) == nil then
         global.grp[gid] = nil
      end
      global.rgrp[eid] = nil
      global.rgid[eid] = nil
   ...
   for ueid, _ in pairs(update) do
      local ugid = global.rgrp[ueid]
      if ugids[ugid] == nil then
         ugids[ugid] = true
      end -- TODO remove condition
   end
   for ugid, _ in pairs(ugids) do
      table.sort(
         global.grp[ugid], function(a, b)
